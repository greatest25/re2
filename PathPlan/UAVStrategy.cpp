...
UAVStrategy::UAVStrategy(QObject* parent) : QObject(parent), m_isTrackingMode(false)
{
    // 初始化无人机追踪模式
...
// 设置追踪模式或巡逻模式
void UAVStrategy::setTrackingMode(bool isTracking)
{
...
// 获取当前模式(追踪或巡逻)
bool UAVStrategy::isTrackingMode() const
{
    return m_isTrackingMode;
}

// 更新敌方无人机位置信息
void UAVStrategy::updateEnemyPositions(const QMap<QString, QPoint>& enemyPositions)
{
    m_enemyPositions = enemyPositions;
}

// 更新敌方无人机位置和血量信息
void UAVStrategy::updateEnemyInfo(const QMap<QString, QPoint>& enemyPositions, const QMap<QString, int>& enemyHp)
{
    m_enemyPositions = enemyPositions;
    m_enemyHp = enemyHp;
}
...
// 为特定无人机单独设置追踪模式
void UAVStrategy::setDroneTrackingMode(const QString& droneId, bool isTracking)
{
...
// 获取特定无人机的追踪模式
bool UAVStrategy::isDroneTracking(const QString& droneId) const
{
    return m_droneTrackingMode.value(droneId, false);
}

// 获取最优追踪目标点 - 优先选择血量高的敌方无人机，血量相同时优先选择R1>R2>R3
QPoint UAVStrategy::getBestTrackingTarget(const QString& droneId) const
{
